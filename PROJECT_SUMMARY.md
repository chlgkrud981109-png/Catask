...
### 1. 나로 AI 어시스턴트 (Naro AI Assistant)
- **Claude API 연동**: 최신 `claude-haiku-4-5-20251001` 모델을 연동하여 지능적인 답변을 제공합니다.
- **보안 프록시**: Cloudflare Workers(`narotask-api.chlgkrud981109.workers.dev`)를 통해 API 키를 환경변수(`NaroTask_API`)로 안전하게 관리합니다.
- **데이터 정규화**: Worker 응답 포맷을 `{ text: "..." }`로 정규화하여 프론트엔드 연동 안정성을 확보했습니다.
- **최적화된 UI**: 채팅 패널의 불필요한 헤더를 제거하고 닫기(X) 버튼만 남겨 깔끔한 인터페이스를 구현했습니다.
- **위젯 연동 준비**: AI 답변을 통해 즉시 보드 위젯을 생성할 수 있는 `appendWidgetPreview` 기능을 개발 중입니다.

### 2. 멀티 보드 & 커스터마이징 (Multi-Board System)
...
### 3. Notion 스타일 계층형 구조 (Hierarchy System)
...
### 4. 고도화된 위젯 시스템 (Advanced Widgets)
...
### 5. 통합 뮤직 플레이어 (YouTube Music Integration)
...
- **AI/Backend**: Anthropic Claude API (`claude-haiku-4-5-20251001`), Cloudflare Workers
...
### 📝 향후 로드맵 (Roadmap)
- [ ] **메모지 크기 조절**: 위젯의 크기를 자유롭게 리사이징하는 기능
- [ ] **나로 AI 드래그 이동**: 에이전트 캐릭터를 페이지 어디로든 이동시키는 기능
- [ ] **보드 탭 리네임 (Rename)**: 탭 영역 더블 클릭 시 즉시 이름 변경 기능 고도화
- [ ] **프리셋 저장 기능**: 보드 설정(배경색, 도트 컬러)을 프리셋으로 저장하고 불러오는 기능 (최대 3개)
- [ ] **구글 로그인 (Google Auth)**: 구글 계정 연동 및 프로필 이미지 동기화 (추후 예정)
